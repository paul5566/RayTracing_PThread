...
void *raytracing( void* arg){
	PTH_NODE* nPtr = (PTH_NODE*)arg;

	point3 u, v, w, d;
    color object_color = { 0.0, 0.0, 0.0 };

	// calculate u, v, w 
    calculateBasisVectors(u, v, w, (nPtr->argPtr)->view);

    idx_stack stk;
	int i,j,k;

    int factor = sqrt(SAMPLES);
	for( j = nPtr->init_height; j < (nPtr->argPtr)->height; j += (nPtr->argPtr)->pthNum){
		for( i = 0; i < (nPtr->argPtr)->width; i++){
			double r = 0, g = 0, b = 0;
			for( k=0; k<SAMPLES; k++){
				idx_stack_init(&stk);
				rayConstruction(d, u, v, w,
                                ...
                                j * factor + k % factor,
                                (nPtr->argPtr)->view,
                                (nPtr->argPtr)->width * factor, 
								(nPtr->argPtr)->height * factor);
				if (ray_color( ((nPtr->argPtr)->view)->vrp, 0.0, d, &stk, (nPtr->argPtr)->rectangulars, (nPtr->argPtr)->spheres,
                              (nPtr->argPtr)->lights, object_color,
                              MAX_REFLECTION_BOUNCES)) {
                    r += object_color[0];
                    g += object_color[1];
                    b += object_color[2];
					...
					r += ((nPtr->argPtr)->background_color)[0];
                    g += ((nPtr->argPtr)->background_color)[1];
                    b += ((nPtr->argPtr)->background_color)[2];
				}
				((nPtr->argPtr)->pixels)[((i + (j * (nPtr->argPtr)->width)) * 3) + 0] = r * 255 / SAMPLES;
                ((nPtr->argPtr)->pixels)[((i + (j * (nPtr->argPtr)->width)) * 3) + 1] = g * 255 / SAMPLES;
                ((nPtr->argPtr)->pixels)[((i + (j * (nPtr->argPtr)->width)) * 3) + 2] = b * 255 / SAMPLES;
			}
		}
	}
	pthread_exit(NULL);
